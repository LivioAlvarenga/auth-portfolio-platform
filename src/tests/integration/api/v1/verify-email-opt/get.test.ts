...
describe('GET /api/v1/verify-email-opt', () => {
  describe('Get Verify Email Use Case', () => {
    test('should return 404 if user not found', async () => {
      ...
      const response = await fetch(
        `${webserver.host}/api/v1/verify-email-opt?token=${userId}&opt=${opt}`,
        {
          method: 'GET',
        },
      )
    ...
    test('should return 404 if token not found or expired', async () => {
      ...
      const response = await fetch(
        `${webserver.host}/api/v1/verify-email-opt?token=${user.id}&opt=${opt}`,
        {
          method: 'GET',
        },
      )
    ...
    test('should return 400 if opt is invalid', async () => {
      ...
      const response = await fetch(
        `${webserver.host}/api/v1/verify-email-opt?token=${token.identifier}&opt=${opt}`,
        {
          method: 'GET',
        },
      )
    ...
    test('should verify email and delete token if opt is valid', async () => {
      ...
      const response = await fetch(
        `${webserver.host}/api/v1/verify-email-opt?token=${token.identifier}&opt=${token.opt}`,
        {
          method: 'GET',
        },
      )
    ...
    test('should delete all expired tokens', async () => {
      ...
      await fetch(
        `${webserver.host}/api/v1/verify-email-opt?token=${user.id}&opt=123456`,
        {
          method: 'GET',
        },
      )
